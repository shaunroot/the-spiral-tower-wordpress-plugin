...
(function () {
    ...
    const config = {
        ...
        continuousStep: 12,       // Pixels per frame when holding
        animationDuration: 1.5,  // Duration of smooth scrolling in seconds
        boundsBuffer: 60,        // Buffer to prevent seeing edge (increase to avoid black space)
    ...
    // REPLACE YOUR CALCULATE BOUNDS FUNCTION WITH THIS
    ...
    function calculateBounds() {
        ...
        // CORRECTED APPROACH: For wide images, we need a larger buffer
        // Calculate adaptive buffer based on aspect ratio
        const aspectRatio = wrapperWidth / wrapperHeight;
        const defaultBuffer = config.boundsBuffer;

        // Increase buffer for wider images - the wider the image, the larger the buffer needed
        let adaptiveBuffer = defaultBuffer;
        if (aspectRatio > 2) { // Very wide images
            adaptiveBuffer = defaultBuffer * 3;
        } else if (aspectRatio > 1.5) { // Moderately wide images
            adaptiveBuffer = defaultBuffer * 2;
        }

        log("Using adaptive buffer:", {
            aspectRatio,
            defaultBuffer,
            adaptiveBuffer
        });
        ...
        if (state.scrollDirection === 'horizontal' || state.scrollDirection === 'both') {
            ...
            // Apply buffer to the bounds calculation
            // For horizontal scrolling on wide images, we need more restriction
            // The math here is reversed from what you might expect:
            // - A larger buffer means less scrolling range (more restricted)
            // - A smaller buffer means more scrolling range (less restricted)
            const bufferScaled = adaptiveBuffer / state.scale;
            const xMax = Math.max(0, effectiveHalfExcess - bufferScaled);
            const xMin = Math.min(0, -effectiveHalfExcess + bufferScaled);
        ...
        if (state.scrollDirection === 'vertical' || state.scrollDirection === 'both') {
            ...
            // Apply buffer to the bounds calculation
            const bufferScaled = adaptiveBuffer / state.scale;
            const yMax = Math.max(0, effectiveHalfExcess - bufferScaled);
            const yMin = Math.min(0, -effectiveHalfExcess + bufferScaled);
