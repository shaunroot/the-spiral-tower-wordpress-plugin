...
SpiralTower.core = (function() {
    ...
    async function init() {
        ...
        if (!checkModulesLoaded()) {
            console.error("Not all modules are loaded. Cannot initialize properly.");
            return;
        }
        
        // Run initialization for each module
        try {
            if (SpiralTower.utils && typeof SpiralTower.utils.init === 'function') await SpiralTower.utils.init();
            if (SpiralTower.scroll && typeof SpiralTower.scroll.init === 'function') await SpiralTower.scroll.init();
            if (SpiralTower.youtube && typeof SpiralTower.youtube.init === 'function') await SpiralTower.youtube.init();
            if (SpiralTower.transitions && typeof SpiralTower.transitions.init === 'function') await SpiralTower.transitions.init();
            
            // Set up CSS and overlay
            setupCss();
            ...
            // Initial setup for YouTube and scrolling
            if (SpiralTower.youtube && typeof SpiralTower.youtube.updateSoundToggleVisuals === 'function') {
                SpiralTower.youtube.updateSoundToggleVisuals(true);
            }
            
            // Setup scrolling for initial page
            setTimeout(() => {
    ...
    // Listen for the custom event from the loader when all modules are loaded
