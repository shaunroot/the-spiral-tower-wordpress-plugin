...
class Spiral_Tower_Floor_Manager
{
    ...
    public function __construct()
    {
        ...
        add_action('add_meta_boxes_floor', array($this, 'add_floor_number_meta_box'));
        add_action('add_meta_boxes_floor', array($this, 'add_custom_script_meta_box')); // <-- ADDED HOOK
    ...
    /**
     * Display Floor Number Meta Box Content
     */
    ...
    /**
     * Save all meta data for the floor post type (MODIFIED METHOD)
     */
    public function save_floor_meta($post_id)
    {
        // --- Security Checks ---
        if (!isset($_POST['floor_meta_nonce']) || !wp_verify_nonce($_POST['floor_meta_nonce'], 'floor_meta_nonce_action')) { return; }
        if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) { return; }
        if (!current_user_can('edit_post', $post_id)) { return; }
        if ('floor' !== get_post_type($post_id)) { return; }

        // --- Save ORIGINAL Floor Settings Fields ---
        $fields_to_save = [
            ...
            '_title_color'              => isset($_POST['title_color']) ? sanitize_text_field($_POST['title_color']) : null,
            '_title_background_color'   => isset($_POST['title_background_color']) ? sanitize_text_field($_POST['title_background_color']) : null,
            '_content_color'            => isset($_POST['content_color']) ? sanitize_text_field($_POST['content_color']) : null,
            '_content_background_color' => isset($_POST['content_background_color']) ? sanitize_text_field($_POST['content_background_color']) : null,
            '_floor_number_color'       => isset($_POST['floor_number_color']) ? sanitize_text_field($_POST['floor_number_color']) : null,
        ];
    ...
    {
        if (!isset($postarr['post_type']) || $postarr['post_type'] !== 'floor') { return $maybe_empty; }
        $floor_number = isset($_POST['floor_number']) ? sanitize_text_field($_POST['floor_number']) : '';
        if (empty($floor_number)) { return $maybe_empty; }

        $args = array(
            'post_type' => 'floor', 'posts_per_page' => 1, 'meta_key' => '_floor_number',
            'meta_value' => $floor_number, 'post__not_in' => isset($postarr['ID']) ? array($postarr['ID']) : array(),
        );
    ...
    {
        // This function might need adjustment based on exact capability needs and testing
        // map_meta_cap handles much of this if capability_type is set correctly.
        // Consider simplifying or removing if map_meta_cap works as expected.
        if (!isset($args[0]) || (strpos($args[0], 'floor') === false && $args[0] !== 'edit_post')) { return $allcaps; }
        $post_id = isset($args[2]) ? $args[2] : 0;
        $user_id = isset($args[1]) ? $args[1] : 0;
        if (!$post_id || !$user_id) { return $allcaps; }
        $post = get_post($post_id);
        if (!$post || $post->post_type !== 'floor') { return $allcaps; }
        $user = get_userdata($user_id);
        if (!$user || !in_array('floor_author', (array)$user->roles)) { return $allcaps; } // Check only floor authors

        // Floor authors can only edit their own posts
        if ($post->post_author != $user_id) {
             if (strpos($args[0], 'edit_') !== false) { $allcaps[$args[0]] = false; }
             if (strpos($args[0], 'delete_') !== false) { $allcaps[$args[0]] = false; }
        }
        return $allcaps;
    }
    ...
    {
        // This applies ONLY if using ACF for the _floor_number field.
        // If using the native meta box, this function has no effect on it.
        if (!isset($field['name']) || $field['name'] !== '_floor_number') { return $field; }
        $user = wp_get_current_user();
        if (in_array('administrator', (array) $user->roles) || in_array('editor', (array) $user->roles)) { return $field; }
        $field['readonly'] = true;
        return $field;
    }
    ...
    public function display_floor_author_dashboard_widget()
    {
        ...
        $args = array(
            'post_type' => 'floor', 'author' => $user_id, 'posts_per_page' => -1, 'post_status' => 'publish'
        );
        $floors = get_posts($args);

        if (empty($floors)) {
            echo '<p>You have not created any floors yet.</p>'; return;
        }
    ...
    public function custom_toolbar_for_floor_authors($wp_admin_bar)
    {
        ...
        if (in_array('floor_author', (array) $user->roles) && !current_user_can('edit_others_floors')) {
            ...
            $wp_admin_bar->add_node(array(
                'id' => 'your_floors_admin_bar', 'title' => 'Your Floors',
            ...
            $floors = get_posts(array(
                'post_type' => 'floor', 'author' => $user->ID, 'posts_per_page' => 5, 'post_status' => 'publish'
            ));
            foreach ($floors as $floor) {
                $floor_number = get_post_meta($floor->ID, '_floor_number', true);
                $wp_admin_bar->add_node(array(
...
/**
 * Instantiate the Floor Manager.
 * Ensure this runs only once, e.g., within your plugin's main file or theme's functions.php.
 */
// new Spiral_Tower_Floor_Manager();